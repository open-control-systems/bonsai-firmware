...
ProjectPipeline::ProjectPipeline() {
...
#ifdef CONFIG_BONSAI_FIRMWARE_SENSOR_LDR_ANALOG_ENABLE
    ldr_sensor_config_storage_ =
        system_pipeline_->get_storage_builder().make(ldr_sensor_config_storage_id_);
    configASSERT(ldr_sensor_config_storage_);

    ldr_sensor_config_.reset(new (std::nothrow) sensor::AnalogConfig(
        *ldr_sensor_config_storage_, CONFIG_BONSAI_FIRMWARE_SENSOR_LDR_ANALOG_VALUE_MIN,
        CONFIG_BONSAI_FIRMWARE_SENSOR_LDR_ANALOG_VALUE_MAX));
    configASSERT(ldr_sensor_config_);

    analog_config_store_->add(*ldr_sensor_config_, ldr_sensor_id_);
    ...
    ldr_sensor_config_store_.reset(new (std::nothrow) sensor::AnalogConfigStore());
    configASSERT(ldr_sensor_config_store_);

    ldr_sensor_config_store_->add(*ldr_sensor_config_, ldr_sensor_id_);

    ldr_sensor_config_store_handler_.reset(
        new (std::nothrow) pipeline::httpserver::AnalogConfigStoreHandler(
            ...
            *ldr_sensor_config_store_, ldr_config_handler_path_));
    configASSERT(ldr_sensor_config_store_handler_);
#endif // CONFIG_BONSAI_FIRMWARE_SENSOR_LDR_ANALOG_ENABLE

#ifdef CONFIG_BONSAI_FIRMWARE_SENSOR_SOIL_ANALOG_RELAY_ENABLE
    soil_relay_sensor_config_storage_ = system_pipeline_->get_storage_builder().make(
        soil_relay_sensor_config_storage_id_);
    configASSERT(soil_relay_sensor_config_storage_);

    soil_relay_sensor_config_.reset(new (std::nothrow) sensor::AnalogConfig(
        *soil_relay_sensor_config_storage_,
    ...
    soil_relay_sensor_config_store_.reset(new (std::nothrow) sensor::AnalogConfigStore());
    configASSERT(soil_relay_sensor_config_store_);

    soil_relay_sensor_config_store_->add(*soil_relay_sensor_config_,
                                         soil_relay_sensor_id_);

    soil_relay_sensor_config_store_handler_.reset(
        new (std::nothrow) pipeline::httpserver::AnalogConfigStoreHandler(
            system_pipeline_->get_func_scheduler(), http_pipeline_->get_server(),
            *ldr_sensor_config_store_, soil_relay_config_handler_path_));
    configASSERT(soil_relay_sensor_config_store_handler_);
#endif // CONFIG_BONSAI_FIRMWARE_SENSOR_SOIL_ANALOG_RELAY_ENABLE

#ifdef CONFIG_BONSAI_FIRMWARE_SENSOR_SOIL_ANALOG_ENABLE
    soil_sensor_config_storage_ =
        system_pipeline_->get_storage_builder().make(soil_sensor_config_storage_id_);
    configASSERT(soil_sensor_config_storage_);

    soil_sensor_config_.reset(new (std::nothrow) sensor::AnalogConfig(
        *soil_sensor_config_storage_, CONFIG_BONSAI_FIRMWARE_SENSOR_SOIL_ANALOG_VALUE_MIN,
    ...
    soil_sensor_config_store_.reset(new (std::nothrow) sensor::AnalogConfigStore());
    configASSERT(soil_sensor_config_store_);

    soil_sensor_config_store_->add(*soil_sensor_config_, soil_sensor_id_);

    soil_sensor_config_store_handler_.reset(
        new (std::nothrow) pipeline::httpserver::AnalogConfigStoreHandler(
            system_pipeline_->get_func_scheduler(), http_pipeline_->get_server(),
            *soil_sensor_config_store_, soil_config_handler_path_));
    configASSERT(soil_sensor_config_store_handler_);
#endif // CONFIG_BONSAI_FIRMWARE_SENSOR_SOIL_ANALOG_ENABLE
